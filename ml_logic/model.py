...
def model_anxiety_disorder(data : pd.DataFrame):
    ...
    #define the model : random forest classifier
    rf_anxiety_disorder = RandomForestClassifier('colsample_bytree': 0.9,
                                                'learning_rate': 0.01,
                                                'max_depth': 7,
                                                'n_estimators': 100,
                                                'subsample': 0.9)
    ...
    #train the model
    rf_anxiety_disorder.fit(X_preprocessed, y_preprocessed)

    return rf_anxiety_disorder
...
def model_mood_disorder(data : pd.DataFrame):
    ...
    xgb_mood_disorder = XGBClassifier('colsample_bytree': 0.8,
                                        'learning_rate': 0.1,
                                        'max_depth': 7,
                                        'n_estimators': 200,
                                        'subsample': 0.9)
...
def model_trauma_and_stress_related_disorder(data : pd.DataFrame):
    ...
    #define the model : xgBoost
    xgb_trauma = XGBClassifier('criterion': 'entropy',
                                'max_depth': 5,
                                'max_features': 'sqrt',
                                'n_estimators': 200)

    #train the model
    xgb_trauma.fit(X_preprocessed, y_preprocessed)
